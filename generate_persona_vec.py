...
def get_hidden_p_and_r(model, tokenizer, prompts, responses, layer_list=None):
    ...
    for text, prompt in tqdm(zip(texts, prompts), total=len(texts)):
        inputs = tokenizer(text, return_tensors="pt", add_special_tokens=False).to(model.model.embed_tokens.weight.device)
        prompt_len = len(tokenizer.encode(prompt, add_special_tokens=False))
        with torch.inference_mode():
            outputs = model.model(**inputs, output_hidden_states=True, use_cache=False)
        ...
        del inputs
    for layer in layer_list:
        prompt_avg[layer] = torch.cat(prompt_avg[layer], dim=0).mean(0).float()
        prompt_last[layer] = torch.cat(prompt_last[layer], dim=0).mean(0).float()
        response_avg[layer] = torch.cat(response_avg[layer], dim=0).mean(0).float()
...
def save_persona_vector(model_name, pos_path, neg_path, trait, save_dir, threshold=50):
    model = AutoModelForCausalLM.from_pretrained(model_name, device_map="balanced", max_memory={0: "22GiB", 1: "22GiB"})
    ...
    persona_effective_prompt_avg_diff = torch.stack([persona_effective_prompt_avg["pos"][l] - persona_effective_prompt_avg["neg"][l] for l in range(len(persona_effective_prompt_avg["pos"]))], dim=0)
    persona_effective_response_avg_diff = torch.stack([persona_effective_response_avg["pos"][l] - persona_effective_response_avg["neg"][l] for l in range(len(persona_effective_response_avg["pos"]))], dim=0)
    persona_effective_prompt_last_diff = torch.stack([persona_effective_prompt_last["pos"][l] - persona_effective_prompt_last["neg"][l] for l in range(len(persona_effective_prompt_last["pos"]))], dim=0)
