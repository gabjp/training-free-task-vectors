...
def load_eval_model(model_name):
    ...
    return model, tokenizer, processor, config.model_type


def apply_chat_template(tokenizer, messages, processor=None, model_type=None):
...
def get_input_device(model, model_type=None):
    if model_type in {"gemma4", "mistral3"}:
        return model.model.language_model.embed_tokens.weight.device

    return model.device


def sample_steering(model, tokenizer, conversations,  vector, layer, coef, bs=20, top_p=1, max_tokens=1000, temperature=1, min_tokens=1, steering_type="response", processor=None, model_type=None, attn_only=False):
    ...
    for messages in conversations:
        prompts.append(apply_chat_template(tokenizer, messages, processor=processor, model_type=model_type))
    ...
    for i in trange(0, len(prompts), bs):
        ...
        input_device = get_input_device(model, model_type=model_type)
        tokenized_batch = {k: v.to(input_device) for k, v in tokenized_batch.items()}
        with ActivationSteerer(model, vector, coeff=coef, layer_idx=layer-1, positions=steering_type, attn_only=attn_only):
            with torch.no_grad():
...
def sample(model, tokenizer, conversations, top_p=1, max_tokens=1000, temperature=1, min_tokens=1, lora_path=None, bs=20, processor=None, model_type=None):
    tokenizer.padding_side = "left"
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token
        tokenizer.pad_token_id = tokenizer.eos_token_id
...
class Question():
    ...
    async def eval(self, llm, tokenizer, coef, vector=None, layer=None, max_tokens=1000, n_per_question=100, steering_type="last", lora_path=None, processor=None, model_type=None, attn_only=False):
        paraphrases, conversations = self.get_input(n_per_question)
        if coef != 0:
            prompts, answers = sample_steering(llm, tokenizer, conversations, vector, layer, coef, temperature=self.temperature, max_tokens=max_tokens, steering_type=steering_type, processor=processor, model_type=model_type, attn_only=attn_only)
        else:
            prompts, answers = sample(llm, tokenizer, conversations, temperature=self.temperature, max_tokens=max_tokens, lora_path=lora_path, processor=processor, model_type=model_type)
        df = pd.DataFrame([
...
async def eval_batched(questions, llm, tokenizer, coef, vector=None, layer=None, n_per_question=100, max_concurrent_judges=100, max_tokens=1000, steering_type="last", lora_path=None, processor=None, model_type=None, attn_only=False):
    ...
    if coef != 0:
        prompts, answers = sample_steering(llm, tokenizer, all_conversations, vector, layer, coef, temperature=questions[0].temperature, max_tokens=max_tokens, steering_type=steering_type, processor=processor, model_type=model_type, attn_only=attn_only)
    else:
        prompts, answers = sample(llm, tokenizer, all_conversations, temperature=questions[0].temperature, max_tokens=max_tokens, lora_path=lora_path, processor=processor, model_type=model_type)
...
def main(model, trait, output_path, coef=0, vector_path=None, layer=None, steering_type="response", max_tokens=1000, n_per_question=10, batch_process=True, max_concurrent_judges=100, persona_instruction_type=None, assistant_name=None, judge_model="gpt-4.1-mini-2025-04-14", version="extract", overwrite=False, attn_only=False):
    ...
    if n_per_question == 1:
        ...
        temperature = 1.0
    llm, tokenizer, processor, model_type = load_eval_model(model)
    ...
    if coef != 0:
        ...
        if attn_only:
            # CLI layers start at 1, dictionary layers start at 0.
            ...
            if len(matching_keys) != 1:
                raise KeyError(
                    f"Expected exactly one key ending with "
                    f"'{key_suffix}', found: {matching_keys}"
                )

            vector = vectors[matching_keys[0]]
            print(f"Using steering vector: {matching_keys[0]}")
        else:
    ...
    if batch_process:
        ...
        outputs_list = asyncio.run(eval_batched(questions, llm, tokenizer,coef, vector, layer, n_per_question, max_concurrent_judges, max_tokens, steering_type=steering_type, lora_path=lora_path, processor=processor, model_type=model_type, attn_only=attn_only))
        outputs = pd.concat(outputs_list)
    else:
        outputs = []
        for question in tqdm(questions, desc=f"Processing {trait} questions"):
            outputs.append(asyncio.run(question.eval(llm, tokenizer,coef, vector, layer, max_tokens, n_per_question, steering_type=steering_type, lora_path=lora_path, processor=processor, model_type=model_type, attn_only=attn_only)))
